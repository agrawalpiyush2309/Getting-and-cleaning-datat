...
# 12. Read activity data
activities<- read.table("activity_labels.txt",col.names=c("id","name"))

# 13. replacing activity ids with their description
tidy_data[,2]<-activities$name[match(unlist(tidy_data[,2]), activities$id)]

# 14. Making the output dataset tidier
tidier_data<-tidy_data %>% gather(Features,Reading,3:59) 
